defmodule DungeonCrawlWeb.Editor.TileTemplateViewTest do
  use DungeonCrawlWeb.ConnCase, async: true

  alias DungeonCrawlWeb.Editor.TileTemplateView
  ...
  test "activate_or_new_version_button/2 renders activate if tile_template inactive", %{conn: conn} do
    ...
    assert Regex.match?(~r{Activate}, inspect(TileTemplateView.activate_or_new_version_button(conn, tile_template)))
  end
  ...
  test "activate_or_new_version_button/2 renders nothing if a new version already exists", %{conn: conn} do
    ...
    refute TileTemplateView.activate_or_new_version_button(conn, tile_template)
  end

  test "activate_or_new_version_button/2 renders new_version if tile template is active", %{conn: conn} do
    tile_template = insert_tile_template(%{active: true})
    assert Regex.match?(~r{New Version}, inspect(TileTemplateView.activate_or_new_version_button(conn, tile_template)))
  end
  ...
  test "error_pre_tag/2 is like the error_tag, but uses a pre tag instead of default span" do
    ...
    assert Phoenix.HTML.safe_to_string(TileTemplateView.error_pre_tag(changeset, :script)) ==
