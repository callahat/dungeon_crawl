defmodule DungeonCrawlWeb.ManageDungeonControllerTest do
  ...
  describe "with an admin user" do
    ...
    test "deletes chosen instance", %{conn: conn} do
      ...
      dungeon = Repo.preload(instance, :dungeon).dungeon
      conn = delete conn, manage_dungeon_path(conn, :delete, dungeon, instance_id: instance.id)
      assert redirected_to(conn) == manage_dungeon_path(conn, :show, dungeon)
      assert Dungeon.get_map(dungeon.id)
      refute Dungeon.get_map(dungeon.id).deleted_at
