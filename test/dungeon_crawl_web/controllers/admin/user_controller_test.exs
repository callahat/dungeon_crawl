defmodule DungeonCrawlWeb.Admin.UserControllerTest do
  ...
  test "shows chosen resource", %{conn: conn, user: _user} do
    ...
    conn = get conn, admin_user_path(conn, :show, target_user)
  ...
  test "renders form for editing chosen resource", %{conn: conn, user: _user} do
    target_user = insert_user @valid_attrs
    conn = get conn, admin_user_path(conn, :edit, target_user)
  ...
  test "updates chosen resource and redirects when data is valid", %{conn: conn, user: _user} do
    target_user = insert_user @valid_attrs
    conn = put conn, admin_user_path(conn, :update, target_user), user: @valid_attrs
    assert redirected_to(conn) == admin_user_path(conn, :show, target_user)
  ...
  test "does not update chosen resource and renders errors when data is invalid", %{conn: conn, user: _user} do
    target_user = insert_user @valid_attrs
    conn = put conn, admin_user_path(conn, :update, target_user), user: @invalid_attrs
    assert html_response(conn, 200) =~ "Edit user"
  end
  ...
  test "deletes chosen resource", %{conn: conn, user: _user} do
    target_user = insert_user @valid_attrs
    conn = delete conn, admin_user_path(conn, :delete, target_user)
    assert redirected_to(conn) == admin_user_path(conn, :index)
