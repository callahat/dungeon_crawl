defmodule DungeonCrawlWeb.Editor.EquipmentControllerTest do
  ...
  describe "non registered users" do
    test "redirects non admin users", %{conn: conn} do
      conn = get conn, edit_equipment_path(conn, :index)
  ...
  describe "registered users who are not admins" do
    ...
    test "renders form for new resources", %{conn: conn} do
      conn = get conn, edit_equipment_path(conn, :new)
      assert html_response(conn, 200) =~ "New item"
    end

    test "creates resource and redirects when data is valid", %{conn: conn} do
      conn = post conn, edit_equipment_path(conn, :create), item: @valid_attrs
    ...
    test "does not create resource and renders errors when data is invalid", %{conn: conn} do
      conn = post conn, edit_equipment_path(conn, :create), item: @invalid_attrs
      assert html_response(conn, 200) =~ "New item"
    end
    ...
    test "renders page not found when id is nonexistent", %{conn: conn} do
      assert_error_sent 404, fn ->
        get conn, edit_equipment_path(conn, :show, -1)
      end
    end
    ...
    test "updates chosen resource and redirects when data is valid", %{conn: conn} do
      ...
      assert redirected_to(conn) == edit_equipment_path(conn, :show, target_item)
      assert Repo.get_by(Item, Map.merge(@valid_attrs,@update_attrs))
    end

    test "does not update chosen resource and renders errors when data is invalid", %{conn: conn} do
      target_item = insert_item(%{user_id: conn.assigns.current_user.id})
      conn = put conn, edit_equipment_path(conn, :update, target_item), item: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit item"
    end
    ...
    test "deletes chosen resource on delete", %{conn: conn} do
      target_item = insert_item(%{user_id: conn.assigns.current_user.id})
      conn = delete conn, edit_equipment_path(conn, :delete, target_item)
      assert redirected_to(conn) == edit_equipment_path(conn, :index)
  ...
  describe "admin users" do
    ...
    test "lists users entries on index", %{conn: conn} do
      items_with_one_of_each_ownership_type(conn.assigns.current_user)
      conn = get conn, edit_equipment_path(conn, :index, %{list: "mine"})
      assert html_response(conn, 200) =~ "Listing items"
      assert html_response(conn, 200) =~ "My Item"
      refute html_response(conn, 200) =~ "Someone elses item"
      refute html_response(conn, 200) =~ "Unowned item"
    ...
    test "lists unowned entries on index", %{conn: conn} do
      items_with_one_of_each_ownership_type(conn.assigns.current_user)
      conn = get conn, edit_equipment_path(conn, :index, %{list: "nil"})
      assert html_response(conn, 200) =~ "Listing items"
    ...
    test "creates resource and redirects when data is valid and can own", %{conn: conn} do
      conn = post conn, edit_equipment_path(conn, :create), item: @valid_attrs, self_owned: "true"
      assert redirected_to(conn) == edit_equipment_path(conn, :index)
      new_item = Repo.get_by(Item, @valid_attrs)
      assert new_item
   ...
   test "can edit active item", %{conn: conn} do
     ...
     conn = get conn, edit_equipment_path(conn, :edit, item), item: @update_attrs
     assert html_response(conn, 200) =~ "Edit item"
   end

    test "updates chosen resource and redirects when data is valid", %{conn: conn} do
      target_item = insert_item @valid_attrs
      conn = put conn, edit_equipment_path(conn, :update, target_item), item: @update_attrs, self_owned: "true"
      assert redirected_to(conn) == edit_equipment_path(conn, :show, target_item)
    ...
    test "updates chosen resource and redirects when data is valid and can assign to no one", %{conn: conn} do
      target_item = insert_item @valid_attrs
      conn = put conn, edit_equipment_path(conn, :update, target_item), item: @update_attrs, self_owned: "false"
      assert redirected_to(conn) == edit_equipment_path(conn, :show, target_item)
      updated_item = Repo.get_by(Item, Map.merge(@valid_attrs,@update_attrs))
      assert updated_item
