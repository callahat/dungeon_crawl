defmodule DungeonCrawlWeb.CrawlerControllerTest do
  ...
  test "show when current dungeon", %{conn: conn, user: user} do
    ...
    DungeonCrawl.Repo.update! DungeonCrawl.Dungeon.MapSet.changeset(map_set, %{active: true, autogenerated: false})
  ...
  # detail
  @tag login_as: "maxheadroom"
  test "detail when dungeon not eligible for detail", %{conn: conn} do
    instance = insert_autogenerated_dungeon_instance()
    map_set = Repo.preload(instance, [map_set: :map_set]).map_set.map_set
    conn = get conn, crawler_path(conn, :detail, map_set.id)
    assert redirected_to(conn) == crawler_path(conn, :show)
  end

  @tag login_as: "maxheadroom"
  test "detail when no current dungeon", %{conn: conn} do
    instance = insert_autogenerated_dungeon_instance()
    map_set = Repo.preload(instance, [map_set: :map_set]).map_set.map_set
    DungeonCrawl.Repo.update! DungeonCrawl.Dungeon.MapSet.changeset(map_set, %{active: true, autogenerated: false})
    conn = get conn, crawler_path(conn, :detail, map_set.id)
    assert html_response(conn, 200) =~ "Join New Instance"
    assert html_response(conn, 200) =~ "Scores"
  end

  @tag login_as: "maxheadroom"
  test "detail when in current dungeon",%{conn: conn, user: user} do
    instance = insert_autogenerated_dungeon_instance()
    map_set = Repo.preload(instance, [map_set: :map_set]).map_set.map_set
    DungeonCrawl.Repo.update! DungeonCrawl.Dungeon.MapSet.changeset(map_set, %{active: true, autogenerated: false})
    insert_player_location(%{map_instance_id: instance.id, user_id_hash: user.user_id_hash})
    conn = get conn, crawler_path(conn, :detail, map_set.id)
    refute html_response(conn, 200) =~ "Join New Instance"
    assert html_response(conn, 200) =~ "Scores"
  end
