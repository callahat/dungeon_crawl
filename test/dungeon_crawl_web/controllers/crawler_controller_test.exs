defmodule DungeonCrawlWeb.CrawlerControllerTest do
  ...
  test "join redirects to show", %{conn: conn} do
    map_set_instance = insert_autogenerated_map_set_instance()
    conn = post conn, crawler_path(conn, :join), map_set_instance_id: map_set_instance.id
    assert redirected_to(conn) == crawler_path(conn, :show)
  end

  test "join redirects to show when starting a new instance", %{conn: conn} do
    map_set = insert_autogenerated_map_set()
    conn = post conn, crawler_path(conn, :join), map_set_id: map_set.id
    assert redirected_to(conn) == crawler_path(conn, :show)
  end
  ...
  test "join does not start a new instance if max_instances is set and hit", %{conn: conn} do
    ...
    conn = post conn, crawler_path(conn, :join), map_set_id: map_set_instance.map_set_id
    assert redirected_to(conn) == crawler_path(conn, :show)
  ...
  test "join instance fails if dungeon is private", %{conn: conn} do
