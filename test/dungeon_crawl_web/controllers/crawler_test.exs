defmodule DungeonCrawlWeb.CrawlerTest do
  ...
  test "join_and_broadcast/4 joining a dungeon" do
    ...
    {:ok, instance} = MapSets.instance_process(tile.level.dungeon_instance_id, tile.level_instance_id)
    location_tile_id = tile.id
    assert %Instances{player_locations: %{^location_tile_id => ^location}} = InstanceProcess.get_state(instance)

    # cleanup
    MapSetRegistry.remove(MapSetInstanceRegistry, tile.level.dungeon_instance_id)
  end
...
#    assert %{tiles: [%{row: tile.row, col: tile.col, rendering: "<div>@</div>"}]} == payload
    ...
    # It registers the player location
    {:ok, instance} = MapSets.instance_process(di.id, instance.id)
    location_tile_id = tile.id
    assert %Instances{player_locations: %{^location_tile_id => ^location}} = InstanceProcess.get_state(instance)

    # cleanup
    MapSetRegistry.remove(MapSetInstanceRegistry, di.id)
  end
  ...
  test "leave_and_broadcast" do
    ...
    {:ok, instance} = MapSets.instance_process(di.id, level_instance.id)
    ...
    junk_pile = Instances.get_tile(state, location.tile)
