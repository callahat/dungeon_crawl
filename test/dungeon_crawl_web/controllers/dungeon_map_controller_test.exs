defmodule DungeonCrawlWeb.DungeonMapControllerTest do
  ...
  describe "map edge with a registered user" do
    ...
    test "gets the adjacent edge tiles", %{conn: conn, dungeon: dungeon} do
      ...
      expected_json = Enum.map(0..20, fn i -> %{"html" => "<div> </div>", "id" => "east_#{ i }"} end)
      conn = get conn, dungeon_map_path(conn, :map_edge, other_dungeon.map_set_id, edge: "east", level_number: other_dungeon.number)
      assert json_response(conn, 200) == expected_json

      conn = get conn, dungeon_map_path(conn, :map_edge, other_dungeon.map_set_id, edge: "north", level_number: other_dungeon.number)
