defmodule DungeonCrawl.ScoresTest do
  ...
  describe "scores" do
    ...
    test "top_scores_for_dungeon/1 returns scores when dungeon given is nil" do
    ...
    test "top_scores_for_dungeon/1 returns scores for given dungeon" do
      score_fixture()
      {:ok, score} = Scores.create_score(Map.merge(@valid_attrs, %{dungeon_id: nil}))
      assert Scores.top_scores_for_dungeon(nil) == [%{Map.put(score, :dungeon, nil) | place: 1}]
    end
    ...
    test "create_score/1 with valid data creates a score" do
      ...
      assert %{duration: 123,
               ...
               victory: true,
               deaths: 1} = score
      # Dungeon ID is optional, no dungeon id indicates it was an autogenerated dungeon
      assert {:ok, %Score{}} = Scores.create_score(@valid_attrs)
    end
