defmodule DungeonCrawl.DungeonTest do
  ...
  describe "dungeons" do
    ...
    test "list_dungeons_with_player_count/0 returns all dungeons preloaded with the players in the map instances" do
      ...
      preloaded_dungeon = Repo.preload(Repo.preload(map, :dungeon).dungeon, :map_instances)
    ...
    test "new_map_version/1 creates a new version of the map" do
      map = insert_autogenerated_dungeon()
      new_version = Dungeon.new_map_version(map)
      assert %Map{} = new_version
      ...
      new_tiles = Repo.preload(new_version, :dungeon_map_tiles).dungeon_map_tiles |> Enum.map(fn(t) -> Elixir.Map.take(t, [:row, :col, :z_index, :tile_template_id]) end)
