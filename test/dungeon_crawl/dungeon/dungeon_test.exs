defmodule DungeonCrawl.DungeonTest do
  ...
  describe "dungeons" do
    ...
    test "list_dungeons_with_player_count/0 returns all dungeons with count of players in them" do
      map = map_fixture()
      assert Dungeon.list_dungeons_with_player_count() == [%{dungeon: map, player_count: 0}]
      insert_player_location(%{dungeon_id: map.id})
      insert_player_location(%{dungeon_id: map.id, user_id_hash: "different"})
      assert Dungeon.list_dungeons_with_player_count() == [%{dungeon: map, player_count: 2}]
    end

    test "list_dungeons_with_player_count/1 returns all dungeons with count of players in them" do
      map = map_fixture()
      autogen_map = map_fixture()
      autogen_map |> Dungeon.change_map() |> Ecto.Changeset.put_change(:autogenerated, true) |> Repo.update!
      insert_player_location(%{dungeon_id: map.id})
      assert Dungeon.list_dungeons_with_player_count(:not_autogenerated) == [%{dungeon: map, player_count: 1}]
    end
  ...
  describe "dungeon_map_tiles" do
    ...
    test "get_map_tiles/2 returns a map_tile from the top in the given direction" do
      ...
      assert Dungeon.get_map_tiles(%{dungeon_id: map_tile.dungeon_id, row: map_tile.row,   col: map_tile.col-1}, "right") == [map_tile, bottom_tile]
    end
    ...
    test "get_map_tiles/4 returns a map tile in the given direction" do
      ...
      assert Dungeon.get_map_tiles(map_tile.dungeon_id, map_tile.row,   map_tile.col-1, "right") == [map_tile, bottom_tile]
    end
  end
end
