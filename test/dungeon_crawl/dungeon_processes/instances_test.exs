defmodule DungeonCrawl.DungeonProcesses.InstancesTest do
  ...
  setup do
    ...
    {_, state} = Instances.create_tile(%Instances{}, tile)
    {_, state} = Instances.create_tile(state, tile_south_1)
    {_, state} = Instances.create_tile(state, tile_south_2)
  ...
  test "get_player_location/2", %{state: state} do
    ...
    {player_tile, state} = Instances.create_player_tile(state, player_tile, location)
  ...
  test "send_event/4", %{state: state} do
    ...
    {_player_tile, state} = Instances.create_player_tile(state, player_tile, player_location)
  ...
  test "create_player_tile/3 creates a player tile and regsiters it" do
    ...
    {new_tile, state} = Instances.create_player_tile(%Instances{}, new_tile, location)
    ...
    assert {^new_tile, state} = Instances.create_player_tile(state, Map.put(new_tile, :character, "O"), location)
    assert %{id: 1, character: "@"} = new_tile
    assert %Instances{
      program_contexts: %{},
      map_by_ids: %{1 =>  Map.put(new_tile, :parsed_state, %{entry_col: 4, entry_row: 5})},
  ...
  test "create_tile/2 creates a tile" do
    ...
    {new_tile, state} = Instances.create_tile(%Instances{}, new_tile)
    ...
    assert {new_tile_1, updated_state} = Instances.create_tile(state, Map.merge(new_tile, %{id: nil, row: 5}))
    ...
    assert {^new_tile, ^state} = Instances.create_tile(state, Map.put(new_tile, :character, "O"))
    assert %{id: 1, character: "M"} = new_tile
    ...
    assert capture_log(fn ->
             assert {_tile, updated_state} = Instances.create_tile(state, tile_bad_script)
    ...
    # If there's a program that starts, adds the tile_id to new_pids, so Instances._cycle_program
    ...
    {new_tile, state} = Instances.create_tile(%Instances{}, tile_good_script)
  ...
  test "set_tile_id/3" do
    ...
    {_tile, state} = Instances.create_tile(%Instances{}, tile)
    ...
    {new_tile, state} = Instances.create_tile(state, new_tile)
    assert new_tile.id == "new_0"

    # noop if the new id is not and int or the old id is not a binary
    assert ^state = Instances.set_tile_id(state, %{id: "new_0"}, "new_0")
    assert ^state = Instances.set_tile_id(state, %{id: 1}, 2)

    # stuff gets updated
    assert updated_state = Instances.set_tile_id(state, Map.put(new_tile, :id, 1), new_tile.id)
  ...
  test "update_tile_state/3", %{state: state} do
    tile = Instances.get_tile(state, %{id: 999, row: 1, col: 2})
    assert {tile, state} = Instances.update_tile_state(state, tile, %{hamburders: 4})
    assert tile.parsed_state[:hamburders] == 4
    assert tile.state == "hamburders: 4"

    assert {tile, _state} = Instances.update_tile_state(state, tile, %{coffee: 2})
    assert tile.parsed_state[:hamburders] == 4
  ...
  test "update_tile/3 updates the tile", %{state: state} do
    ...
    assert {updated_tile, updated_state} = Instances.update_tile(state, tile, new_attributes)
  ...
  test "update_tile/3", %{state: state} do
    tile_id = 999
    assert {_updated_tile, state} = Instances.update_tile(state, %{id: tile_id}, %{id: 11111, character: "X", row: 1, col: 1})
    ...
    assert {_updated_tile, state} = Instances.update_tile(state, %{id: tile_id}, %{row: 2, col: 3})
    ...
    {_another_tile, state} = Instances.create_tile(state, another_tile)

    # Won't move to the same z_index
    assert {_updated_tile, state} = Instances.update_tile(state, %{id: tile_id}, %{row: 5, col: 6})
    assert %Tile{id: ^tile_id, character: "X", row: 2, col: 3, z_index: 0} = state.map_by_ids[tile_id]
    assert {_updated_tile, state} = Instances.update_tile(state, %{id: tile_id}, %{row: 5, col: 6, z_index: 1})
    ...
    # Move the new tile
    assert {_updated_tile, state} = Instances.update_tile(state, %{id: -3}, %{character: "M"})
    ...
    assert {_updated_tile, state} = Instances.update_tile(state, %{id: -3}, %{script: "#END\n:TOUCH\n?s"})
    ...
    assert {_updated_tile, state} = Instances.update_tile(state, %{id: 999}, %{script: "?n"})
  ...
  test "delete_player_tile/2 deletes the tile and unregisters player location", %{state: state} do
    ...
    assert {_deleted_tile, state} = Instances.delete_tile(state, tile)
  ...
  test "delete_player_tile/3 removes the tile and unregisters player location, but does not mark for deletion", %{state: state} do
    ...
    assert {_deleted_tile, state} = Instances.delete_tile(state, tile, false) # false parameter
    refute state.map_by_ids[tile_id]
    refute state.player_locations[tile_id]
    %Instances{ program_contexts: programs,
                map_by_ids: by_id,
  ...
  test "direction_of_tile/3" do
    ...
    {_, state} = Instances.create_tile(%Instances{}, tile_nw)
    {_, state} = Instances.create_tile(state, tile_n)
    {_, state} = Instances.create_tile(state, tile_ne)
    {_, state} = Instances.create_tile(state, tile_w)
    {_, state} = Instances.create_tile(state, tile_me)
    {_, state} = Instances.create_tile(state, tile_e)
    {_, state} = Instances.create_tile(state, tile_sw)
    {_, state} = Instances.create_tile(state, tile_s)
    {_, state} = Instances.create_tile(state, tile_se)

    # If target is at same coords as object, idle
    assert Instances.direction_of_tile(state, tile_me, tile_me) == "idle"
    # Target inline but is blocked, still returns the direction
    assert Instances.direction_of_tile(state, tile_me, tile_n) == "north"
    assert Instances.direction_of_tile(state, tile_me, tile_w) == "west"
    # Target inline and unblocked
    assert Instances.direction_of_tile(state, tile_me, tile_s) == "south"
    assert Instances.direction_of_tile(state, tile_me, tile_e) == "east"
    # Target is in a diagonal direction, preference given to the non blocked
    assert Instances.direction_of_tile(state, tile_me, tile_ne) == "east"
    assert Instances.direction_of_tile(state, tile_me, tile_sw) == "south"
    # Target is in a diagonal direction, non blocked, one is picked
    assert Enum.member?(["south", "east"], Instances.direction_of_tile(state, tile_me, tile_se))
    # Target is in a diagonal direction, both blocked, one is picked
    assert Enum.member?(["north", "west"], Instances.direction_of_tile(state, tile_me, tile_nw))
  end
  ...
  test "is_player_tile?/2" do
    ...
    {player_tile, state} = Instances.create_player_tile(%Instances{}, player_tile, location)
    {other_tile, state} = Instances.create_tile(state, other_tile)

    assert Instances.is_player_tile?(state, player_tile)
    refute Instances.is_player_tile?(state, other_tile)
  ...
  test "subtract/4 health on a tile" do
    ...
    state = %Instances{dungeon_instance_id: 14, instance_id: 12345}
    {wall, state} = Instances.create_tile(state, wall)
    {breakable, state} = Instances.create_tile(state, breakable)
    {damageable, state} = Instances.create_tile(state, damageable)
    ...
    refute Instances.get_tile_by_id(updated_state, breakable)
    ...
    assert Instances.get_tile_by_id(updated_state, damageable).parsed_state[:health] == 5
    ...
    refute Instances.get_tile_by_id(updated_state, damageable)
  ...
  test "subtract/4 non health on a tile" do
    ...
    {tile, state} = Instances.create_tile(%Instances{}, tile)

    assert {:ok, updated_state} = Instances.subtract(state, :cash, 5, tile.id)
    assert Instances.get_tile_by_id(updated_state, tile).parsed_state[:cash] == 0
    assert {:not_enough, updated_state} = Instances.subtract(state, :cash, 6, tile.id)
    assert state == updated_state
    assert {:not_enough, updated_state} = Instances.subtract(state, :nothing, 1, tile.id)
    assert state == updated_state
  end
  ...
  test "subtract/4 health on a player tile" do
    ...
    state = %Instances{dungeon_instance_id: 14, instance_id: 123}
    {player_tile, state} = Instances.create_player_tile(state, player_tile, location)
  ...
  test "subtract/4 health on a player tile when instance reset_player_when_damaged sv is true" do
    ...
    location = %Location{id: 444, user_id_hash: "dubs", tile_instance_id: 123}
    state = %Instances{dungeon_instance_id: 14, instance_id: 123, state_values: %{reset_player_when_damaged: true}}
    {player_tile, state} = Instances.create_player_tile(state, player_tile, location)
    {player_tile, state} = Instances.update_tile_state(state, player_tile, %{entry_row: 1, entry_col: 9})

    assert {:ok, updated_state} = Instances.subtract(state, :health, 10, player_tile.id)

    player_tile = Instances.get_tile_by_id(updated_state, player_tile)
  ...
  test "subtract/4 non health on a player tile" do
    ...
    assert Instances.get_tile_by_id(updated_state, player_tile).parsed_state[:gems] == 0
    assert Enum.member? updated_state.dirty_player_tile_stats, player_tile.id
  ...
  test "gameover/3 - ends game for all players in instance" do
    ...
    state = %Instances{state_values: %{rows: 20, cols: 20}, dungeon_instance_id: instance.dungeon_instance_id}
    {player_tile_1, state} = Instances.create_player_tile(state, player_tile_1, player_location_1)
    {player_tile_2, state} = Instances.create_player_tile(state, player_tile_2, player_location_2)
    ...
    assert %{parsed_state: %{gameover: true, score_id: ^score_1_id}} =
      Instances.get_tile_by_id(updated_state, player_tile_1)
    assert %{parsed_state: %{gameover: true, score_id: ^score_2_id}} =
      Instances.get_tile_by_id(updated_state, player_tile_2)
    assert_receive %Phoenix.Socket.Broadcast{
  ...
  test "gameover/4 - ends game for given player" do
    ...
    assert %{parsed_state: %{gameover: true, score_id: ^score_id}} =
      Instances.get_tile_by_id(updated_state, player_tile_1)
    refute Instances.get_tile_by_id(updated_state, player_tile_2).parsed_state[:gameover]
    assert_receive %Phoenix.Socket.Broadcast{
            topic: ^player_channel_1,
            event: "gameover",
