defmodule DungeonCrawl.InstanceRegistryTest do
  ...
  alias DungeonCrawl.DungeonProcesses.{InstanceRegistry,InstanceProcess,Instances,ProgramRegistry,ProgramProcess}
  ...
  test "create/2", %{instance_registry: instance_registry} do
    ...
    assert %Instances{program_registry: program_registry,
    ...
    assert [map_tile.id] == ProgramRegistry.list_all_program_ids(program_registry)
    %{program: program} = ProgramRegistry.lookup(program_registry, map_tile.id)
                          |> ProgramProcess.get_state()

    assert program == %Program{broadcasts: [],
                               instructions: %{1 => [:halt, [""]],
                                               2 => [:noop, "TOUCH"],
                                               3 => [:text, ["*PimPom*"]]},
                               labels: %{"touch" => [[2, true]]},
    ...
    on_exit(fn -> InstanceRegistry.remove(DungeonInstanceRegistry, instance.id) end)
  end
  ...
  test "create/3/4", %{instance_registry: instance_registry} do
    ...
    assert [] == ProgramRegistry.list_all_program_ids(program_registry)
    ...
    assert %Instances{map_by_ids: by_ids,
                      map_by_coords: by_coords,
    ...
    on_exit(fn -> InstanceRegistry.remove(DungeonInstanceRegistry, map_tile.map_instance_id) end)
  end
  ...
  test "create safely handles a dungeon instance that does not exist in the DB", %{instance_registry: instance_registry} do
    ...
    log = ExUnit.CaptureLog.capture_log(fn -> InstanceRegistry.create(instance_registry, instance.id); :timer.sleep(5); Logger.flush() end)
    assert :error = InstanceRegistry.lookup(instance_registry, instance.id)
