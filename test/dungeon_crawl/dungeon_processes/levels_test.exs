defmodule DungeonCrawl.DungeonProcesses.LevelsTest do
  ...
  test "gameover/3 - ends game for all players in instance" do
    ...
    # doesn't score when a player abandons the game and its not an autogenerated instance
    assert state == Levels.gameover(state, player_tile_1.id, true, "Gave Up")
    assert Scores.list_scores == []

    # scores when its an autogenerated dungeon and player abandons or idles out
    dungeon = DungeonProcess.get_dungeon(dungeon_process)
    DungeonProcess.set_dungeon(dungeon_process, %{dungeon | autogenerated: true})
    Levels.gameover(state, player_tile_1.id, true, "Idled Out")
    assert [%{result: "Idled Out, Level: 0"}] = Scores.list_scores

    Scores.list_scores |> Enum.map(&(Repo.delete(&1)))

    Levels.gameover(state, player_tile_1.id, true, "Dead")
    assert [%{result: "Dead, Level: 0"}] = Scores.list_scores
