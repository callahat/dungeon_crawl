defmodule DungeonCrawl.Scripting.CommandTest do
  ...
  test "get_command/1" do
    ...
    assert Command.get_command(:change_map_set_instance_state) == :change_map_set_instance_state
  ...
  test "BECOME a SLUG the author cannot use ignores the slug" do
    ...
    {map_tile, state} = Instances.create_map_tile(%Instances{}, %MapTile{id: 123, row: 1, col: 2, character: ".", map_instance_id: 1, state: ""})

    tile = insert_tile_template(%{character: "!", state: "blocking: true", active: true, public: false, user_id: other_user.id})
    public_tile = insert_tile_template(%{character: "!", state: "blocking: true", active: true, user_id: other_user.id, public: true})

    # no author, so autogenerated and can use any slug
    runner_state = %Runner{object_id: map_tile.id, state: state}
    %Runner{state: updated_state} = Command.become(runner_state, [%{slug: tile.slug}])
    assert %{character: "!", state: "blocking: true"} = Instances.get_map_tile_by_id(updated_state, map_tile)

    # author is admin
    runner_state = %Runner{object_id: map_tile.id, state: %{ state | author: admin_author}}
    %Runner{state: updated_state} = Command.become(runner_state, [%{slug: tile.slug}])
    assert %{character: "!", state: "blocking: true"} = Instances.get_map_tile_by_id(updated_state, map_tile)

    # author also created the tile
    runner_state = %Runner{object_id: map_tile.id, state: %{ state | author: other_user}}
    %Runner{state: updated_state} = Command.become(runner_state, [%{slug: tile.slug}])
    assert %{character: "!", state: "blocking: true"} = Instances.get_map_tile_by_id(updated_state, map_tile)

    # author not admin, nor tile creator but tile is public
    runner_state = %Runner{object_id: map_tile.id, state: %{ state | author: author}}
    %Runner{state: updated_state} = Command.become(runner_state, [%{slug: public_tile.slug}])
    assert %{character: "!", state: "blocking: true"} = Instances.get_map_tile_by_id(updated_state, map_tile)

    # author not admin, nor tile creator and  tile is not public
    runner_state = %Runner{object_id: map_tile.id, state: %{ state | author: author}}
    assert %Runner{state: updated_state} = Command.become(runner_state, [%{slug: tile.slug}])
    assert %{character: ".", state: ""} = Instances.get_map_tile_by_id(updated_state, map_tile)
