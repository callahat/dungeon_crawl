defmodule DungeonCrawl.Scripting.CommandTest do
  ...
  alias DungeonCrawl.DungeonProcesses.{Levels, LevelProcess, DungeonRegistry, DungeonProcess, Registrar}
  ...
  test "BECOME" do
    ...
    assert updated_tile.parsed_state == %{health: 20}
  end
  ...
  test "BECOME a SLUG the author cannot use ignores the slug" do
    ...
    {tile, state} = Levels.create_tile(%Levels{}, %Tile{id: 123, row: 1, col: 2, character: ".", level_instance_id: 1, state: ""})
  ...
  test "BECOME a SLUG" do
    ...
    refute updated_tile.parsed_state == tile.parsed_state
    ...
    assert %{blocking: true} = updated_tile.parsed_state
    ...
    assert updated_tile.parsed_state == %{blocking: false}
  ...
  test "COMPOUND_MOVE into something blocking (or a nil square) triggers a THUD event" do
    ...
    assert %{status: :wait,
             ...
             pc: 1,
             lc: 0,
             messages: [{"THUD", %{tile_id: nil, parsed_state: %{}}}]
           } = program
  end
...
#    stubbed_object = %{state: "thing: true", parsed_state: %{thing: true}}
  ...
  test "LOCK" do
    ...
    assert tile.parsed_state == %{locked: true}
  end
  ...
  test "MOVE into something blocking (or a nil square) triggers a THUD event" do
    ...
    assert %{status: :wait,
             ...
             broadcasts: [],
             pc: 1,
             messages: [{"THUD", %{tile_id: 4, parsed_state: %{blocking: true}}}]
           } = program
  end
...
#    assert new_tile.slug == squeaky_door.slug
  ...
  test "RANDOM" do
    ...
    updated_tile = Levels.get_tile_by_id(updated_state, tile)
    assert Enum.member?(["a", "b", "c"], updated_tile.parsed_state[:a])
  ...
  test "SEND message to tiles in a direction" do
    ...
    {obj, state} = Levels.create_tile(state, %Tile{id: 1337, character: "c", row: 2, col: 2, z_index: 0, state: "facing: north"})
    sender = %{tile_id: obj.id, parsed_state: obj.parsed_state, name: nil}
  ...
  test "SEND message to tiles by name" do
    ...
    {_, state}   = Levels.create_tile(state, %Tile{id: 999,  name: "C", character: "c", row: 3, col: 2, z_index: 0, script: "#END"})
    {obj, state} = Levels.create_tile(state, %Tile{id: 1337, name: nil, character: "c", row: 2, col: 2, z_index: 0})
    sender = %{tile_id: obj.id, parsed_state: %{}, name: nil}
  ...
  test "SEND message to tiles by id" do
    ...
    assert updated_state.program_messages == [{"new_2", "name", %{tile_id: 1337, name: nil, parsed_state: %{}}}]
  end
  ...
  test "SEQUENCE" do
    ...
    updated_tile = Levels.get_tile_by_id(updated_state, tile)
    assert updated_tile.parsed_state[:c] == "red"
...
#    obj = %{obj | parsed_state: %{facing: "north"}}
  ...
  test "TARGET_PLAYER" do
    ...
    target_tile = Levels.get_tile_by_id(updated_state, %{id: object_tile.parsed_state[:target_player_map_tile_id]})
  ...
  test "UNEQUIP" do
    ...
    Already at full health
    """
    {losing_tile, state} = Levels.create_tile(%Levels{}, %Tile{id: 1, character: "E", row: 1, col: 1, z_index: 0, state: "health: 1, equipment: gun, equipped: gun"})
    ...
    assert map[losing_tile.id].parsed_state[:equipped] == nil
    ...
    assert map[losing_tile.id].parsed_state[:equipped] == "gun"
  ...
  test "UNEQUIP of an equipped item sets a differen item as equipped when able" do
    ...
    Already at full health
    """
    {losing_tile, state} = Levels.create_tile(%Levels{}, %Tile{id: 1, character: "E", row: 1, col: 1, z_index: 0, state: "health: 1, equipment: gun #{other_item.slug}, equipped: gun"})
    {giver, state} = Levels.create_tile(state, %Tile{id: 3, character: "c", row: 2, col: 1, z_index: 1, state: "thing: gun", script: script, color: "red"})
    ...
    assert map[losing_tile.id].parsed_state[:equipment] == [other_item.slug]
    assert map[losing_tile.id].parsed_state[:equipped] == other_item.slug
  end
  ...
  test "UNLOCK" do
    ...
    assert tile.parsed_state == %{locked: false}
  end
