defmodule DungeonCrawl.Action.MoveTest do
  ...
  test "moving to an empty floor space" do
    ...
    dungeon = insert_stubbed_dungeon(%{}, [%{row: 1, col: 2, tile_template_id: floor.id, z_index: 0}])
    player_location = insert_player_location(%{dungeon_id: dungeon.id, row: 1, col: 2})
    destination = %MapTile{dungeon_id: dungeon.id, row: 1, col: 1, tile_template_id: floor.id, z_index: 0}
