defmodule DungeonCrawl.Action.MoveTest do
  ...
  test "moving to an empty floor space" do
    ...
    {floor_a, state} = Instances.create_tile(%Instances{}, floor_a)
    {_floor_b, state} = Instances.create_tile(state, floor_b)
    {player_location, state} = Instances.create_tile(state, player_location)

    destination =     Instances.get_tile(state, %{row: 1, col: 1})
    ...
    assert Instances.get_tiles(state, %{row: 1, col: 2}) == [floor_a]
    assert Instances.get_tiles(state, %{row: 1, col: 1}) == [new_location,
                                                                 destination]
  ...
  test "moving to an empty floor space from a non tile" do
    ...
    {floor_b, state} = Instances.create_tile(%Instances{}, floor_b)
    {player_location, state} = Instances.create_tile(state, player_location)
    ...
    assert Instances.get_tiles(state, %{row: 1, col: 2}) == []
    assert Instances.get_tiles(state, %{row: 1, col: 1}) ==
             [new_location,
              floor_b]
  end
  ...
  test "moving to a bad space" do
    ...
    {wall, state} = Instances.create_tile(%Instances{}, wall)
    {player_location, state} = Instances.create_tile(state, player_location)
  ...
  test "moving to a teleporter" do
    ...
    {floor_a, state} =      Instances.create_tile(%Instances{state_values: %{rows: 7, cols: 7}}, floor_a)
    {teleporter_e, state} = Instances.create_tile(state, teleporter_e)
    {floor_b, state} =      Instances.create_tile(state, floor_b)
    {wall_b, state} =       Instances.create_tile(state, wall_b)
    {_teleporter_w, state} = Instances.create_tile(state, teleporter_w)
    {floor_c, state} =      Instances.create_tile(state, floor_c)
    {wall_c, state} =       Instances.create_tile(state, wall_c)
    ...
    {_, state} = Instances.delete_tile(state, wall_c, false)
    ...
    assert Instances.get_tiles(updated_state, %{row: 1, col: 1}) == [floor_a]
    assert Instances.get_tiles(updated_state, %{row: 1, col: 5}) == [new_location,
                                                                         floor_c]

    # right past the teleporter counts as nearest available when its available
    {_, state} = Instances.delete_tile(state, wall_b, false)
    ...
    assert Instances.get_tiles(updated_state, %{row: 1, col: 1}) == [floor_a]
    assert Instances.get_tiles(updated_state, %{row: 1, col: 3}) == [new_location,
                                                                         floor_b]

    # cannot use teleporter if coming from the side
    {player_location, state} = Instances.update_tile(state, player_location, %{row: 2, col: 2})
    assert {:invalid} = Move.go(player_location, teleporter_e, state)

    # can push objects through teleporter
    {player_location, state} = Instances.update_tile(state, player_location, %{row: 1, col: 0})
    ball = %Tile{id: 800, row: 1, col: 1, z_index: 1, character: "o", state: "blocking: true, pushable: true"}
    {ball, state} =       Instances.create_tile(state, ball)

    assert {:ok, %{{1, 1} => new_location, {1, 0} => _old_location, {1, 3} => pushed_location}, updated_state} =
      Move.go(player_location, ball, state)
    assert Instances.get_tiles(updated_state, %{row: 1, col: 0}) == []
    assert Instances.get_tiles(updated_state, %{row: 1, col: 1}) == [new_location,
                                                                         floor_a]
    assert Instances.get_tiles(updated_state, %{row: 1, col: 3}) == [pushed_location,
                                                                         floor_b]
  end
  ...
  test "pushing an object" do
    ...
    {floor, state} = Instances.create_tile(%Instances{}, floor)
    {ball, state} = Instances.create_tile(state, ball)
    {player_location, state} = Instances.create_tile(state, player_location)
    ...
    assert Instances.get_tiles(updated_state, %{row: 1, col: 3}) == []
    assert Instances.get_tiles(updated_state, %{row: 1, col: 2}) == [new_location]
    assert Instances.get_tiles(updated_state, %{row: 1, col: 1}) == [ball_new_location, floor]

    # Something not pushing does not push
    bullet_location   = %Tile{id: 1001,  row: 1, col: 3, z_index: 2, character: "-", state: "not_pushing: true"}
    {bullet_location, state} = Instances.create_tile(state, bullet_location)
  ...
  test "pushing an object directionally wrong way" do
    ...
    {_floor, state} = Instances.create_tile(%Instances{}, floor)
    {ball, state} = Instances.create_tile(state, ball)
    {player_location, state} = Instances.create_tile(state, player_location)
  ...
  test "pushing a line of objects" do
    ...
    {ball, state} = Instances.create_tile(state, ball)
    {player_location, state} = Instances.create_tile(state, player_location)
    {floor2, state} = Instances.create_tile(state, floor2)
    {_ball2, state} = Instances.create_tile(state, ball2)
    ...
    assert Instances.get_tiles(updated_state, %{row: 1, col: 2}) == [new_location]
    assert Instances.get_tiles(updated_state, %{row: 1, col: 1}) == [ball_new_location, floor]
    assert Instances.get_tiles(updated_state, %{row: 1, col: 0}) == [ball2_new_location, floor2]

    # cannot push into blocking
    wall            = %Tile{id: 1001, row: 1, col: 0, z_index: 1, character: "#", state: "blocking: true"}
    {_wall, state} = Instances.create_tile(state, wall)
  ...
  test "a squishable object" do
    ...
    {floor, state} = Instances.create_tile(%Instances{}, floor)
    {balloon, state} = Instances.create_tile(state, balloon)
    {player_location, state} = Instances.create_tile(state, player_location)
    ...
    assert Instances.get_tiles(updated_state, %{row: 1, col: 3}) == []
    assert Instances.get_tiles(updated_state, %{row: 1, col: 2}) == [new_location]
    assert Instances.get_tiles(updated_state, %{row: 1, col: 1}) == [floor]
    ...
    assert Instances.get_tiles(updated_state, %{row: 1, col: 2}) == [new_bullet_location, balloon]
  end
  ...
  test "a squishable pushable object" do
    ...
    assert Instances.get_tiles(updated_state, %{row: 1, col: 3}) == []
    assert Instances.get_tiles(updated_state, %{row: 1, col: 2}) == [new_location]
    assert Instances.get_tiles(updated_state, %{row: 1, col: 1}) == [new_balloon_location, floor]
    ...
    assert Instances.get_tiles(updated_state, %{row: 1, col: 3}) == []
    assert Instances.get_tiles(updated_state, %{row: 1, col: 2}) == []
    assert Instances.get_tiles(updated_state, %{row: 1, col: 1}) == [new_location, floor]
  end

  test "moving to something that is not a tile" do
    player_location   = %Tile{id: 1000,  row: 1, col: 2, z_index: 1, character: "@"}

    {player_location, state} = Instances.create_tile(%Instances{}, player_location)
  ...
  test "can_move/1 is true if the destination exists and is not blocking" do
    ...
    {wall, _state} = Instances.create_tile(%Instances{}, wall)
    {floor_b, _state} = Instances.create_tile(%Instances{}, floor_b)
