defmodule DungeonCrawl.Action.PullTest do
  ...
  setup _config do
    ...
    destination = Instances.get_tile(state, %{row: 1, col: 2})
    puller = Instances.get_tile(state, %{row: 2, col: 2})
  ...
  describe "pull/3" do
    test "pulling but movement is blocked", %{state: state, puller: puller, destination: destination} do
      ...
      {_object, state} = Instances.create_tile(state, object)
      {destination, state} = Instances.update_tile_state(state, destination, %{blocking: true})
    ...
    test "successful pull of one item", %{state: state, puller: puller, destination: destination} do
      ...
      {_object1, state} = Instances.create_tile(state, object1)
      {_object2, state} = Instances.create_tile(state, object2)
    ...
    test "pull a chain of items that form a circuit", %{state: state, puller: puller, destination: destination} do
      ...
      {_object1, state} = Instances.create_tile(state, object1)
      {_object2, state} = Instances.create_tile(state, object2)
      {_object3, state} = Instances.create_tile(state, object3)
      {_object4, state} = Instances.create_tile(state, object4)
    ...
    test "map_tile_id is replaced with the pullers id", %{state: state, puller: puller, destination: destination} do
      ...
      {object1, state} = Instances.create_tile(state, object1)
      {object2, state} = Instances.create_tile(state, object2)
      ...
      assert length(Map.keys(tile_changes)) == 4
      object1 = Instances.get_tile_by_id(updated_state, object1)
      object2 = Instances.get_tile_by_id(updated_state, object2)
  ...
  describe "would_not_pull/2" do
    test "would not pull", %{state: state} do
      ...
      {object1, state} = Instances.create_tile(state, object1)
      {object2, _state} = Instances.create_tile(state, object2)
  ...
  describe "can_pull/3" do
    test "cannot pull", %{state: state, puller: puller, destination: destination} do
      ...
      {object, _state} = Instances.create_tile(state, object)
    ...
    test "pullable but only by a specific tile id", %{state: state, puller: puller, destination: destination} do
      ...
      {object1, state} = Instances.create_tile(state, object1)
      {object2, state} = Instances.create_tile(state, object2)
      {object3, _state} = Instances.create_tile(state, object3)
