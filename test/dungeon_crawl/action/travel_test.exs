defmodule DungeonCrawl.Action.TravelTest do
  ...
  test "passage/3 different instance or level", %{player_location: player_location, level_1: level_1, level_2: level_2, instance_registry: instance_registry} do
    ...
    # travel to floor 2 from floor 1 takes player tile to a spawn coordinate
    {:ok, instance_1} = LevelRegistry.lookup_or_create(instance_registry, level_1.id)
    LevelProcess.run_with(instance_1, fn (state) ->
    ...
    assert_receive %Phoenix.Socket.Broadcast{
        topic: ^player_channel,
        event: "change_level",
        payload: %{level_id: ^level_2_id, level_render: _rendered_dungeon, fade_overlay: fade_overlay}}

    refute fade_overlay # by default no fade overlay when travelling to an adjacent level

    # travel to floor 1 from floor 2 takes player tile to a spawn coordinate
    {:ok, instance_2} = LevelRegistry.lookup_or_create(instance_registry, level_2.id)
    LevelProcess.run_with(instance_2, fn (state) ->
      assert {:ok, state} = Travel.passage(player_location, %{adjacent_level_id: level_1.id, edge: "west"}, state)
      refute Levels.get_tile_by_id(state, %{id: player_location.tile_instance_id})
      {:ok, state}
    end)
    ...
    assert_receive %Phoenix.Socket.Broadcast{
        topic: ^player_channel,
        event: "change_level",
        payload: %{level_id: ^level_1_id, level_render: _rendered_dungeon, fade_overlay: fade_overlay}}

    refute fade_overlay == "" # fade overlay is "on" for level_1
  end
