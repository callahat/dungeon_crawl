defmodule DungeonCrawl.Action.TravelTest do
  ...
  setup do
    ...
    InstanceProcess.run_with(instance_1, fn (state) ->
      {_player_tile, state} = Instances.create_player_tile(state, Repo.preload(player_location, :tile).tile, player_location)
      {:ok, %{ state | passage_exits: [{Instances.get_tile(state, %{row: 6, col: 9}).id,"red"}] }}
    end)
  ...
  test "passage/4 same instance or level", %{player_location: player_location, level_1: level_1, instance_registry: instance_registry} do
    ...
    InstanceProcess.run_with(instance_1, fn (state) ->
      ...
      assert %{row: 6, col: 9, level_instance_id: ^level_1_id} = Instances.get_tile_by_id(state, %{id: player_location.tile_instance_id})
      {:ok, state}
    end)
  ...
  test "passage/4 different instance or level", %{player_location: player_location, level_1: level_1, level_2: level_2, instance_registry: instance_registry} do
    ...
    InstanceProcess.run_with(instance_1, fn (state) ->
      ...
      refute Instances.get_tile_by_id(state, %{id: player_location.tile_instance_id})
      {:ok, state}
    end)
    ...
    {:ok, instance_2} = InstanceRegistry.lookup_or_create(instance_registry, level_2.id)
    InstanceProcess.run_with(instance_2, fn (state) ->
      assert %{row: 1, col: 5, level_instance_id: ^level_2_id} = Instances.get_tile_by_id(state, %{id: player_location.tile_instance_id})
      {:ok, state}
    end)
  ...
  test "passage/3 same instance or level", %{player_location: player_location, level_1: level_1, instance_registry: instance_registry} do
    ...
    InstanceProcess.run_with(instance_1, fn (state) ->
      ...
      level_1_id = level_1.id
      assert %{row: 19, col: 1, level_instance_id: ^level_1_id} = Instances.get_tile_by_id(state, %{id: player_location.tile_instance_id})
      {:ok, state}
    end)
  ...
  test "passage/3 different instance or level", %{player_location: player_location, level_1: level_1, level_2: level_2, instance_registry: instance_registry} do
    ...
    {:ok, instance_2} = InstanceRegistry.lookup_or_create(instance_registry, level_2.id)
    InstanceProcess.run_with(instance_2, fn (state) ->
      assert %{row: 3, col: 0, level_instance_id: ^level_2_id} = Instances.get_tile_by_id(state, %{id: player_location.tile_instance_id})
      {:ok, state}
    end)
