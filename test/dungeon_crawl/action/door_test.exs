defmodule DungeonCrawl.Action.DoorTest do
  ...
  test "open/1 opens a closed door" do
    ...
    dungeon = insert_stubbed_dungeon_instance(%{},
      [Map.merge(%{row: 1, col: 1, tile_template_id: closed_door.id, z_index: 0},Map.take(closed_door, [:character,:color,:background_color]))])
  ...
  test "open/1 is invalid for anything other than a closed door" do
    ...
    dungeon = insert_stubbed_dungeon_instance(%{},
      [Map.merge(%{row: 4, col: 5, tile_template_id: real_fake_door.id, z_index: 0}, Map.take(real_fake_door, [:character,:color,:background_color]))])
  ...
  test "close/1 closes an open door" do
    ...
    dungeon = insert_stubbed_dungeon_instance(%{},
      [Map.merge(%{row: 2, col: 3, tile_template_id: open_door.id, z_index: 0}, Map.take(open_door, [:character,:color,:background_color]))])
  ...
  test "close/1 is invalid for anything other than an open door" do
    ...
    dungeon = insert_stubbed_dungeon_instance(%{},
      [Map.merge(%{row: 8, col: 9, tile_template_id: real_fake_door.id, z_index: 0}, Map.take(real_fake_door, [:character,:color,:background_color]))])
