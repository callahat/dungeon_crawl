defmodule DungeonCrawl.PlayerTest do
  ...
  describe "player_locations" do
    ...
    @valid_attrs %{user_id_hash: "some content", row: 1, col: 1}
    @update_attrs %{row: 1, col: 2}
    ...
    test "create_location/1 with valid data returns a location" do
      ...
      assert {:ok, %Location{} = _location} = Player.create_location(Map.merge(%{dungeon_id: dungeon.id, map_tile_id: player_map_tile.id}, @valid_attrs))
    end

    test "create_location/1 with invalid data returns error changeset" do
      dungeon = insert_autogenerated_dungeon()
      assert {:error, %Ecto.Changeset{}} = Player.create_location(Map.merge(%{dungeon_id: dungeon.id}, @invalid_attrs))
    end
    
    test "create_location!/1 with good params returns a location" do
      dungeon = insert_autogenerated_dungeon()
      player_map_tile = insert_player_map_tile(Map.merge(%{dungeon_id: dungeon.id}, @valid_attrs))
      assert %Location{} = _location = Player.create_location!(Map.merge(%{dungeon_id: dungeon.id, map_tile_id: player_map_tile.id}, @valid_attrs))
    end

    test "update_location/2 with valid data updates the location" do
      location = location_fixture(%{user_id_hash: "getLocation"})

      assert {:ok, location} = Player.update_location(location, @update_attrs)
      assert %Location{} = location
    end

    test "update_location/2 with invalid data returns an error changeset" do
      location = location_fixture(%{user_id_hash: "getLocation"})

      assert {:error, %Ecto.Changeset{}} = Player.update_location(location, @invalid_attrs)
      assert location == Player.get_location!(location.user_id_hash)
    end
