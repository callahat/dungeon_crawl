defmodule DungeonCrawl.PlayerTest do
  ...
  describe "player_locations" do
    ...
    test "create_location_on_spawnable_space/3 returns a location" do
      dungeon_instance = insert_autogenerated_dungeon_instance(%{state: "starting_lives: 3"})
      avatar = %{color: "black", background_color: "whitesmoke", name: "Anon"}
      assert {:ok, %Location{} = location} = Player.create_location_on_spawnable_space(dungeon_instance, "goodhash", avatar)
      ...
      assert {:ok, %Location{} = location} = Player.create_location_on_spawnable_space(dungeon_instance, "differenthash", avatar)
      assert Repo.preload(location, :tile).tile
      assert Repo.preload(location, :tile).tile.character == "@" # its in the basic tile template seeds

      # when player has an account
      user = insert_user()
      assert {:ok, %Location{} = location} = Player.create_location_on_spawnable_space(dungeon_instance, user.user_id_hash, avatar)
    ...
    test "delete_location!/1 deletes the location and deletes the autogenerated dungeon" do
      ...
      level_id = location.tile.level.level_id
      ...
      refute Repo.get(Dungeons.Level, level_id)
      refute Repo.get(DungeonInstances.Level, instance_id)
      refute Repo.get(Dungeons.Dungeon, dungeon.id)
    end
    ...
    test "delete_location!/1 deletes the location but not the non autogen dungeon" do
      ...
      assert %Location{} = Player.delete_location!(location)
      assert_raise Ecto.NoResultsError, fn -> Player.get_location!(location.user_id_hash)  end
      refute Repo.get(DungeonInstances.Dungeon, instance_id)
