defmodule DungeonCrawl.CrawlerController do
  ...
  alias DungeonCrawl.{Crawler,Dungeon,DungeonGenerator,DungeonMapTile,PlayerLocation}
  alias Ecto.Multi
...
#  end
  ...
  def show(conn, _opts) do
    player_location = Repo.get_by(PlayerLocation, %{user_id_hash: get_session(conn, :user_id_hash)}) 
  ...
  def create(conn, _opts) do
    ...
    |> Multi.run(:player_location, fn(%{dungeon: dungeon}) ->
        empty_floor = Repo.preload(dungeon, :dungeon_map_tiles).dungeon_map_tiles
                      ...
                      |> Enum.random
        result = PlayerLocation.changeset(%PlayerLocation{}, %{dungeon_id: dungeon.id,row: empty_floor.row, col: empty_floor.col, user_id_hash: get_session(conn, :user_id_hash)})
                 |> Repo.insert!
        {:ok, result}
      end)
    |> Repo.transaction
