defmodule DungeonCrawl.DungeonProcesses.LevelProcess do
  ...
  def handle_info(:perform_actions, %Levels{count_to_idle: 0} = state) do
    ...
    # if a player left an autogenerated level, then the related db backed level will have been deleted before the write_db
    # check is sent, at which point its safe to shut down the level since there is no going back with a autogen.
    if state.dirty_ids != %{}, do: send(self(), :write_db)

    Process.send_after(self(), :perform_actions, @timeout * 20)
