...
#    InstanceRegistry.link_dungeon(program_registry, self())
  ...
  def handle_call({:scorable?}, _from, %{dungeon: dungeon, state_values: state_values} = state) do
    {:reply, dungeon.active && ! dungeon.autogenerated && ! state_values[:no_scoring], state}
  end
  ...
  def handle_info(:check_for_players, state) do
    if Enum.count(InstanceRegistry.player_location_ids(state.instance_registry)) > 0 do
      ...
      Logger.info "Map Set Process ##{state.dungeon_instance.id} terminating after a period of time with no players"
