defmodule DungeonCrawl.Dungeon do
  ...
  @doc """
  Gets the other map set versions associated with this map set, includes the score count.
  """
  def get_map_sets(line_identifier) do
    Repo.all(from ms in MapSet,
             left_join: s in Score, on: s.map_set_id == ms.id,
             where: ms.line_identifier == ^line_identifier,
             group_by: ms.id,
             order_by: [desc: ms.version],
  ...
  @doc """
  Copies a map set and returns a map of the copied fields. Does not include the id or other record specific keys.

  ## Examples

      iex> copy_map_set_fields(%{id: 1, name: value, ...})
      %{name: value}

      iex> copy_map_set_fields(nil)
      %{}
  """
  def copy_map_set_fields(nil), do: %{}
  def copy_map_set_fields(map_set) do
    Elixir.Map.take(map_set, @copiable_map_set_fields)
  end
  ...
  defp _insert_line_identifier({:ok, map_set}) do
    if map_set.line_identifier do
      {:ok, map_set}
    else
      update_map_set(map_set, %{line_identifier: map_set.id})
    end
  end
  ...
  def create_new_map_set_version(%MapSet{active: true} = map_set) do
    unless next_version_exists?(map_set) do
      case Multi.new
           |> Multi.insert(:new_map_set_version, _map_set_copy(map_set))
           |> Multi.run(:new_maps, fn(_repo, %{new_map_set_version: new_map_set}) ->
                results = Repo.preload(map_set, :dungeons).dungeons
                          |> Enum.map(fn(dungeon) ->
        ...
        {:ok, %{new_map_set_version: new_map_set}} -> {:ok, new_map_set}
  ...
  defp _map_set_copy(map_set) do
    attrs = Elixir.Map.merge(copy_map_set_fields(map_set), %{version: map_set.version+1, previous_version_id: map_set.id})
    MapSet.changeset(%MapSet{}, attrs)
  end

  defp _dungeon_copy_changeset(map, map_set_id) do
    attrs = copy_map_fields(map)
  ...
  @doc """
  Returns a map of the copied map fields. Does not include the id or other record specific keys.

  ## Examples

      iex> copy_map_fields(%{id: 1, name: value, ...})
      %{name: value}

      iex> copy_map_fields(nil)
      %{}
  """
  def copy_map_fields(nil), do: %{}
  def copy_map_fields(map_set) do
    Elixir.Map.take(map_set, @copiable_map_fields)
  end
  ...
  defp _generate_dungeon_map_tiles(dungeon, dungeon_generator) do
    ...
    |> Enum.to_list
    |> Enum.map(fn({{row,col}, tile}) -> Elixir.Map.merge(copy_map_tile_fields(tile_mapping[tile]),
  ...
  defp _adjust_sizing(map, updated_map) do
    ...
    new_dmts = _dim_list_difference(map, updated_map)
               |> Enum.map(fn({row,col}) -> Elixir.Map.merge(copy_map_tile_fields(empty_tile_template),
                                                          %{dungeon_id: updated_map.id,
                                                            row: row,
                                                            col: col,
                                                            z_index: 0,
  ...
  @doc """
  Copies a map tile and returns a map of the copied fields. Does not include the id or other record specific keys.
  ...
  ## Examples
      ...
      {:error, %Ecto.Changeset{}}
  """
  def copy_map_tile_fields(nil), do: %{}
  def copy_map_tile_fields(map_tile) do
    Elixir.Map.take(map_tile, @copiable_map_tile_fields)
  end
