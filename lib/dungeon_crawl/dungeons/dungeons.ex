defmodule DungeonCrawl.Dungeons do
  ...
  Does not return dungeons with preload assocations.
  ...
  unplayed: true
  not_won: true
  name: <binary> (case insensitive, filters on dungeon names as well as dungeon author names)

  Other given filters will be ignored.

  ## Examples

      iex> list_active_dungeons(%{active: true}, "useridhashasdf")
      [%Dungeon{}, ...]

      iex> list_active_dungeons(%{}, "useridhashasdf")
      [%Dungeon{}, ...]
  """
  ...
  defp _filter(query, {:name, name}, _) do
    from [d, user: user] in query,
         where: ilike(d.name, ^"%#{name}%") or ilike(user.name, ^"%#{name}%")
  end

  defp _filter(query, {:favorite, true}, user_id_hash) do
    from d in query,
      where: exists(
  ...
  def generate_dungeon(level_generator, dungeon_attrs \\ %{}, level_attrs \\ %{}, to_be_edited \\ false) do
    case create_dungeon(Map.merge(dungeon_attrs, %{autogenerated: !to_be_edited})) do
      {:ok, dungeon} ->
        level_attrs = Map.merge(level_attrs, %{dungeon_id: dungeon.id, entrance: true})
        case generate_level(level_generator, level_attrs, !to_be_edited) do
          {:ok, _} -> {:ok, dungeon}
          error -> error
        end
      error -> error
  ...
  defp _generate_level_tiles(level, level_generator, for_solo) do
    basic_mapping = TileSeeder.basic_tiles
    advanced_mapping = TileTemplates.autogenerated_dungeon_tile_mapping()
  ...
  defp _generate_tile(tile, level_id, row, col, z_index) do
    Map.merge(copy_tile_fields(tile),
              %{level_id: level_id,
                row: row,
                col: col,
                z_index: z_index,
                tile_template_id: tile.id})
  end
