defmodule DungeonCrawl.DungeonInstances do
  ...
  """
  def create_dungeon(%Dungeons.Dungeon{} = dungeon, is_private \\ false) do
    ...
    |> Multi.run(:levels, fn(_repo, %{dungeon: dungeon_instance}) ->
        result = Repo.preload(dungeon, :levels).levels
                 |> Enum.map(fn(level) ->
                      {:ok, %{level: level}} = create_level(level, dungeon_instance.id)
                      level
                    end)
        {:ok, result}
      end)
  ...
  ## Examples
      ...
      iex> create_level(%Dungeons.Level{})
      {:ok, %{level: %DungeonInstances.Level{}}}

  """
  def create_level(%Dungeons.Level{} = level, di_id) do
    level_attrs = Map.merge(Dungeons.copy_level_fields(level),
                            %{dungeon_instance_id: di_id, level_id: level.id})
    Multi.new()
