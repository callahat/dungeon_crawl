defmodule DungeonCrawl.Player do
  ...
  alias DungeonCrawl.DungeonInstances.{MapSet, Map, MapTile}
  ...
  ## Examples
      ...
      iex> create_location_on_spawnable_space(%Map{})
      {:ok, %Location{}}
  """
  def create_location_on_spawnable_space(%MapSet{} = msi, user_id_hash, user_avatar) do
  ...
  defp _create_map_tile_for_location(%MapSet{} = msi, user_id_hash, user_avatar) do
    ...
    top_tile = DungeonInstances.get_map_tile(entrance.id, spawn_location.row, spawn_location.col)
    z_index = if top_tile, do: top_tile.z_index + 100, else: 0

    player_tile_template = TileTemplates.TileSeeder.player_character_tile()

    Elixir.Map.take(spawn_location, [:row, :col])
    |> Elixir.Map.merge(%{map_instance_id: entrance.id})
    |> Elixir.Map.merge(%{z_index: z_index})
    |> Elixir.Map.merge(TileTemplates.copy_fields(player_tile_template))
    |> Elixir.Map.merge(%{name: user_avatar["name"], color: user_avatar["color"], background_color: user_avatar["background_color"]})
    |> Elixir.Map.put(:name, Account.get_name(user_id_hash))
    |> DungeonInstances.create_map_tile!()
  ...
  def delete_location!(%Location{} = location) do
    ...
    if location.map_tile.dungeon.map_set.autogenerated do
      DungeonCrawl.Dungeons.hard_delete_map_set!(location.map_tile.dungeon.map_set.map_set)
      location
    else
  ...
  ## Examples
      ...
      4
  """
  def players_in_dungeon(%Dungeons.Map{id: dungeon_id}) do
    Repo.one(from m in Dungeons.Map,
  ...
  def players_in_dungeon(%Map{id: instance_id}) do
  ...
  def players_in_dungeon(%{instance_id: instance_id}) do
    players_in_dungeon(%Map{id: instance_id})
  end

  @doc """
  Returns the player locations in a given dungeon instance.
  """
  def players_in_instance(%Map{id: instance_id}) do
