defmodule DungeonCrawlWeb.Crawler do
  ...
  def join_and_broadcast(%Dungeons.Dungeon{} = where, user_id_hash, user_avatar, is_private) do
    ...
    |> Enum.each(fn(level_instance) -> MapSets.instance_process(level_instance.dungeon_instance_id, level_instance.id) end)
  ...
  defp _broadcast_join_event(location) do
    ...
    {:ok, instance} = MapSets.instance_process(tile.level.dungeon_instance_id, tile.level.id)
...
#                                    %{row: top.row, col: top.col, tile: tile})
      DungeonCrawlWeb.Endpoint.broadcast("dungeons:#{instance_state.dungeon_instance_id}:#{tile.level_instance_id}",
                                         "tile_changes",
  ...
  def leave_and_broadcast(%Player.Location{} = location) do
    ...
    {:ok, instance} = MapSets.instance_process(di.id, tile.level_instance_id)
