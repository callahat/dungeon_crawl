defmodule DungeonCrawlWeb.CrawlerController do
  ...
  plug :validate_not_crawling  when action in [:index, :create, :avatar, :validate_avatar, :invite, :validate_invite]
  ...
  def destroy(conn, _opts) do
    ...
    post_leave_path = if dungeon.active || dungeon.autogenerated, do: Routes.crawler_path(conn, :index), else: Routes.dungeon_path(conn, :show, dungeon)
  ...
  defp validate_not_crawling(conn, _opts) do
    if conn.assigns.player_location == nil do
      ...
      conn
      # might not want or need this error flash anymore
  ...
  defp validate_crawling(conn, _opts) do
    if conn.assigns.player_location != nil do
      ...
      conn
      # may not need this error, or want it, depending on if destroy is getting called multiple times somehow
      |> put_flash(:error, "Not currently crawling")
