defmodule DungeonCrawlWeb.CrawlerController do
  ...
  plug :assign_player_location when action in [:show, :detail, :create, :avatar, :validate_avatar, :invite, :validate_invite, :destroy]
  ...
  plug :validate_detailable when action in [:detail]
  ...
  def detail(conn, _opts) do
    player_location = conn.assigns[:player_location]
    map_set = Repo.preload(conn.assigns.map_set, [:dungeons, :user, :map_set_instances])
    owner_name = if map_set.user_id, do: map_set.user.name, else: "<None>"
    title_map = Dungeon.get_title_map(map_set)
                |> Repo.preload(:dungeon_map_tiles)

    scores = Scores.top_scores_for_map_set(map_set.id)
    details = %{who: map_set.name, map_set_id: map_set.id}

    render(conn, "detail.html", map_set: map_set,
                                title_map: title_map,
  ...
  defp validate_detailable(%{params: %{"map_set_id" => map_set_id}} = conn, _opts) do
    map_set = Dungeon.get_map_set(map_set_id)

    if map_set && !map_set.deleted_at && map_set.active && !map_set.autogenerated do
      conn
      |> assign(:map_set, map_set)
    else
      conn
      |> put_flash(:error, "Cannot get details for that dungeon")
      |> redirect(to: Routes.crawler_path(conn, :show))
      |> halt()
    end
  end
